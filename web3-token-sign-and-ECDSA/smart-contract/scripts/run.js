...
async function main() {
  ...
  if (allowlistedAddresses.includes(selectedAddress)) {
    ...
    messageHash = ethers.id(selectedAddress);
  ...
  const contract = await hre.ethers.deployContract("ThreeNft", [baseTokenURI]);
  ...
  let txn;
  txn = await contract.preSale(2, messageHash, signature, { value: ethers.parseEther("1") });
  await txn.wait();
